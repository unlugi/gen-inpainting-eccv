...
parser.add_argument('--image_dir', default='/raid/users/geunlu/datasets/inpainting/still3/test/X/', type=str,
                    help='The folder containing images to be completed.')
parser.add_argument('--mask_dir', default='/raid/users/geunlu/datasets/inpainting/still3/test/Y/', type=str,
...
def complete(image_file):
    ...
    with tf.Session(config=sess_config) as sess:
        ...
        image_small[:h // grid * grid, :w // grid * grid, :] = result[0][:, :, ::-1]
        ...
        if make_small_flag == True:
            ...
            save_value = cv2.imwrite(os.path.join(args.output_dir, image_file), image_small)
