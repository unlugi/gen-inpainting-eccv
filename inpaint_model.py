...
from loss_sensitive_ops import gan_lsgan_loss, gan_glsgan_loss
...
class InpaintCAModel(Model):
    ...
    def build_graph_with_losses(self, batch_data, config, training=True,
        ...
        if config.GAN == 'lsgan_gp':
            # separate gan (local-global)
            ...
            # loss-sensitive gan loss with penalty
            g_loss_local, d_loss_local = gan_lsgan_loss(pos_local, neg_local, local_patch_batch_pos,
                                                        local_patch_batch_complete, config, name='gan/local_gan')
            g_loss_global, d_loss_global = gan_lsgan_loss(pos_global, neg_global, batch_pos, batch_complete,
                                                          config, name='gan/global_gan')
            losses['g_loss'] = config.GLOBAL_WGAN_LOSS_ALPHA * g_loss_global + g_loss_local
            losses['d_loss'] = d_loss_global + d_loss_local
        ...
        if config.GAN == 'glsgan_gp':
            ...
            # loss-sensitive gan loss with penalty
            g_loss_local, d_loss_local = gan_glsgan_loss(pos_local, neg_local, local_patch_batch_pos,
                                                        local_patch_batch_complete, config, name='gan/local_gan')
            g_loss_global, d_loss_global = gan_glsgan_loss(pos_global, neg_global, batch_pos, batch_complete,
                                                          config, name='gan/global_gan')
            losses['g_loss'] = config.GLOBAL_WGAN_LOSS_ALPHA * g_loss_global + g_loss_local
            losses['d_loss'] = d_loss_global + d_loss_local
